...
def ask(prompt_text, default=None, help_text="", icon="", allow_back=True, category="default"):
    while True:
        print(f"\n{Color.YELLOW}╭──────────────────────────────────────────────────────────────────────────────╮")
        for line in help_text.strip().split('\n'):
            print(f"│ {Color.BLUE}{icon} {line.ljust(74)}{Color.END}{Color.YELLOW} │")
        if allow_back:
            print(f"│ {Color.CYAN}{'󰌍  Type \'back\' to return to the previous configuration step.'.ljust(76)}{Color.END}{Color.YELLOW} │")
        ...
        val = pt_prompt(ANSI(f"  {Color.BOLD}{prompt_text}{Color.END}: "), history=cat_history).strip()

        if val.lower() == "back" and allow_back: raise GoBack()
        if val == "" and default is not None: return default
        if val != "": return val
        print(f"  {Color.RED}❌ ERROR: This field cannot be empty.{Color.END}")
    ...
    while True:
        try:
            if step == 1:
                ...
                print(f"│ {Color.BLUE}📂 Opening File Manager for target selection...                           {Color.END}{Color.YELLOW} │")
                print(f"│ {Color.BLUE}💡 Cancelling will automatically select local ./lamedb.                   {Color.END}{Color.YELLOW} │")
